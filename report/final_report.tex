...
\graphicspath{{../outputs/figures/}{figures/}{outputs/figures/}}
...
\begin{subfigure}[t]{0.48\textwidth}
  \centering\includegraphics[width=\linewidth]{02_scree.png}
  \caption{Scree plot. Red dashed line: Kaiser eigenvalue $=1$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
  \centering\includegraphics[width=\linewidth]{02_cumvar.png}
...
$\sigma$-divergence with $\beta$-convergence
(Figure~\ref{fig:dedi-time}).
...
\midrule
East      & 56.09 & 68.31 & $+12.22$\\
Central   & 44.10 & 58.26 & $+14.16$\\
West      & 31.59 & 43.28 & $+11.69$\\
Northeast & 40.03 & 48.11 & $+8.08$\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{subfigure}[t]{0.49\textwidth}
  \centering\includegraphics[width=\linewidth]{03_region_trend.png}
  \caption{Regional DEDI trajectories (yearly mean).}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \centering\includegraphics[width=\linewidth]{03_dispersion.png}
...
transparency.  (iv)~The factorial design treats $310$ province--year observations as independent
replicates, but the same $31$ provinces populate every cell and both period
cells.  Period is therefore a within-subject factor fitted with a
between-subjects error term, and the $310$ rows carry roughly $31$ units of
independent information, so the reported $F$ statistics are anti-conservative
and the region contrasts rest on $3$--$12$ provinces per group rather than
$15$--$60$ observations.  Re-fitting the same model on province$\times$period
means ($n=62$, which removes the year-level replication) leaves every
qualitative conclusion intact --- region Pillai $V=0.927$, $F(12,159)=5.93$,
$p<.001$; period $V=0.895$, $F(4,51)=109.10$, $p<.001$; interaction
$V=0.056$, $F(12,159)=0.25$, $p=.995$ --- while the $F$ statistics fall
substantially (region by a factor of $4.3$, period by $1.8$).  The
Games--Howell contrasts in Section~\ref{sec:results} pool across periods and
share the same assumption, so their $p$-values should be read as descriptive.
A province-level random-effect or cluster-robust extension is left for future
work.
